...
They reject a non-empty output directory before metric computation and never
replace existing result files; choose a new `--output` directory for each run.
...
smoke and intentionally skips FID/FVD distribution scores. For a one-image
continuous smoke, `--frame-only` reads only the first frame of the first clip
and reports paired PSNR/SSIM/LPIPS; it does not report TWE/TDE or FVD. If an
identical GT/prediction pair yields infinite PSNR, smoke JSON represents that
diagnostic value as the string `"Infinity"`; official output refuses
non-finite metrics.
...
$UNILIP_PYTHON csgo_benchmark_v2_eval/run_eval.py smoke continuous \
  --pred-root outputs/csgo_benchmark_v2_seen10/ControlAR/seed_0/continuous/gen_imgs \
...
The synthetic canonical comparison is reproducible with
`$UNILIP_PYTHON csgo_benchmark_v2_eval/validation/canonical_metric_parity.py`;
it writes `validation/canonical_metric_parity_report.json` and removes its
temporary frames. Add `--run-fvd-smoke --device cuda` to run the copied FVD
implementation on two synthetic 16-frame 224×224 clips per side.
